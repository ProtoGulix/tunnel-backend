...
Crée une tâche manuelle. `origin` doit être `resp` ou `tech` — `plan` est réservé à la génération automatique d'occurrences préventives.
...
| `origin`          | string | **oui** | `resp` ou `tech` uniquement (`plan` interdit)            |
