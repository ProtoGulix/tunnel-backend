        ...
        try:
            ...
            try:
                ...
                cur.execute(
                    ...
                    """,
                    (intervention_id, intervention_id, occurrence_id),
                )
                conn.commit()
                logger.info(
    ...
    def repair_orphaned_data(self) -> dict:
        ...
        try:
            ...
            if rows2:
                ...
                logger.info(
                    "Repair Bug 1b : %s intervention_task rattachés via DI", len(
                        rows2)
                )
            ...
            for occ_id, occ_plan_id, linked_intervention_id, occ_status in orphan_rows:
                ...
                new_status = 'in_progress' if occ_status in (
                    'pending', 'generated') else occ_status
